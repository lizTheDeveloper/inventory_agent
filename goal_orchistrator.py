...
Use tmux to launch a detached session, so that when you use the terminal server, you're able to detach from that process, which may be a long-running process. And then make sure to direct the tmux terminal's standard output to somewhere in the directory that you have access to. 
...
async def run(graph_knowledge: MCPServer, filesystem_server: MCPServer, terminal_server: MCPServer):
    ...
    result = await Runner.run(starting_agent=agent, input="Please begin on the goal.", max_turns=50)
        ...
        async with MCPServerStdio(
            ...
            async with MCPServerStdio(
                ...
                with trace(workflow_name="MCP Graph Memory", trace_id=trace_id):
                    ...
                    await run(graph_knowledge_server, filesystem_server, terminal_server)
